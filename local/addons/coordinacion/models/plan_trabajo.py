...
def generar_fecha_hora(fecha, hora_inicio):
    ...
    my_date = datetime(fecha.year, fecha.month, fecha.day, hora_inicio, 0, 0 ,0)
    my_date += delta_hr
    return my_date
        ...
        if(agregarRegistro):
            ...
            if product_line.prestador_id:
                ...
                if product_line.product_id and product_line.product_id.nomenclador_id:
                    ...
                    costo_excepcion = busca_excepcion_costo(self,prestador_id,paciente_id,nomenclador_id,fecha_actual)
            ...
            fecha_hora = generar_fecha_hora(fecha_actual,product_line.hora_inicio)
            dic_linea = {
                'name' : name,
...
def validar_campos(product_line):
    ...
    if ( not product_line.fecha_aux_ini) or (not product_line.fecha_aux_ini):
        raise ValidationError(_("la fecha de inicio y fin tienen que estar definidas"))
    if (product_line.cantidad <= 0):
        raise ValidationError(_("la cantidad de horas no puede se cero o negativa"))
    if( not frecuencia):
...
class PlanTrabajo(models.Model):
    ...
    status = fields.Selection(STATES, default=STATES[0][0],help="Estado del plan de trabajo")

    status_pami = fields.Selection(STATES_PAMI, default=STATES_PAMI[0][0], readonly=True, string='Estado Pami',help="Estado de la OP de Pami")



    producto_ids = fields.One2many('hcd.plan_trabajo_line', 'plan_id', string='Servicios')

    producto_ids_original = fields.One2many('hcd.plan_trabajo_line_original', 'plan_id', string='Prestaciones originales')
    ...
    def _get_filtro_id(self):
        for rec in self:
            rec.filtro_id = 0
            if rec.paciente_id:
                if (rec.paciente_id.hcd_type == 'cliente'):   # or (rec.paga_paciente == 'SI'):
    ...
    def crea_agenda(self):
        for applicant in self:
            ...
            for product_line in lista_ids:
                ...
                if (product_line.estado == 'no_asignado' or product_line.estado == 'primer_visita'):
                    if (product_line.product_id.hcd_categ_prod == 'servicio'):
                        if product_line.prestador_id:  #si tiene prestador asignado, creo_agenda 
                            ...
                            creo_agenda = True    #no asignado,sin prestador , pero es guardia, lo creo en agenda  estado Agendado              
                            estado='agendado'
                    else:   #no es servicio               
                        creo_agenda = True    
                        estado = 'agendado'
                ...
                if creo_agenda:
                    generar_agenda(self, fecha_inicio, fecha_fin, plan_id, paciente_id,empleado_id,product_line,estado)
